...
class ArrowEncoding(BaseBatchFileEncoding):
    """Arrow IPC file batch encoding.

    Registering this subclass (via ``BaseBatchFileEncoding.__init_subclass__``)
    is what allows singer-sdk's ``_process_batch_message`` to decode
    ``{"format": "arrow"}`` at all -- without it, that call raises ``KeyError``
    before the message ever reaches the sink.
    """

    __encoding_format__ = "arrow"
